...
pair<double*, vector<double*>> grpcenterpoint(double node[3] ,int voxelsize,int depth) {
	...
	const double* node_ptr = node;
	grpvisited.insert(node);

	for (int i = 0; i < grppoints.size(); i++) {
		cout << "forrunne3de =============" << endl;
		double cord[3] = { grppoints[i][0],grppoints[i][1],grppoints[i][2] };
		double* cordd = new double[3];
		cordd = cord;
			...
			if (xmin <= cord[0] && xmax >= cord[0]) {
				if (ymin <= cord[1] && ymax >= cord[1]) {
					if (zmin <= cord[2] && zmax >= cord[2]) {
						...
						double cc[3] = { cord[0],cord[1],cord[2] };
						grpvisited.insert(cc);
						cout << "inserted here" <<grpvisited.size()<< endl;
						no_of_points++;
...
cout << endl;
	...
	//filling grpoints(cordinate) from bottom(octant id)
	for (int i = 0; i < bottom.size(); i++) {
		std::vector<double> cord = bmap[bottom[i]].second;
		double linep[3] = { cord[0],cord[1],cord[2] };
		grppoints.push_back(linep);
	/*	cout << grppoints[i][0] << " " << grppoints[i][1] << " " << grppoints[i][2] << endl;*/
		//cout << " grppoints  size is equla to " << grppoints.size() << endl;
	}


	//@@@
	//grppoints from after level 0
	//from 1 layer
	while (depth<=level){
		
		for (size_t i = 0; i < grppoints.size(); ++i) {
			
			/*for (const double* cord : grpvisited) {
				std::cout << "Visited: " << cord[0] << ", " << cord[1] << ", " << cord[2] << std::endl;
			}*/
