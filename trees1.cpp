...
pair<double*,vector<int>> centerpoint(int node,double center[3],int voxelsize) {
	...
	//right
	VoxelData right = rightvoxel(node, center, voxelsize);
	if (right.voxelId != -100  && bmap[right.voxelId].first == 'P') {
		x += right.centerX;
		y += right.centerY;
		z += right.centerZ;
		no_of_points++;
	...
	if (righttop.voxelId != -100 && bmap[righttop.voxelId].first == 'P') {
		x += righttop.centerX;
		y += righttop.centerY;
		z += righttop.centerZ;
		no_of_points++;
	...
	if (rtb.voxelId != -100 && bmap[rtb.voxelId].first == 'P') {
		x += rtb.centerX;
		y += rtb.centerY;
		z += rtb.centerZ;
		no_of_points++;
	...
	VoxelData top = topvoxel(node, center, voxelsize);
	if (top.voxelId != -100 && bmap[top.voxelId].first == 'P') {
		x += top.centerX;
		y += top.centerY;
		z += top.centerZ;
		no_of_points++;
	...
	VoxelData back = backvoxel(node, center, voxelsize);
	if (back.voxelId != -100 && bmap[back.voxelId].first == 'P') {
		x += back.centerX;
		y += back.centerY;
		z += back.centerZ;
		no_of_points++;
	...
	if (bt.voxelId != -100 && bmap[bt.voxelId].first == 'P') {
		x += bt.centerX;
		y += bt.centerY;
		z += bt.centerZ;
		no_of_points++;
	...
	if (right.voxelId != -100 && bmap[br.voxelId].first == 'P') {
		x += br.centerX;
		y += br.centerY;
		z += br.centerZ;
		no_of_points++;
	...
	*/
	...
	for (int i = 0; i < bottom.size(); i++){
		...
		if (bottom_visited.find(bottom[i]) == bottom_visited.end()) {
			...
			// grp 2*2*2
			// xno yno zno
			//pehle wala 1 and next wala 0 in x  101010
			/*int xno, yno, zno;
			int z_layer = (vid - 1) / (oct_in_edge * oct_in_edge);
			zno = z_layer + 1;

			int remaining_index = (vid - 1) % (oct_in_edge * oct_in_edge);

			yno = (remaining_index / oct_in_edge) + 1;
			xno = (remaining_index % oct_in_edge) + 1;*/
			...
			*/
			...
			for (int i = 0; i < joinee.size(); i++) {
				...
				vtkIdType lineIndices[2] = { 0, i+1 }; // Connect Point 4 to Point 0
